...
class kitti(imdb):
    def __init__(self, isFinish):
        imdb.__init__(self, 'kitti')
        self._data_path = '/Users/anekisei/Documents/tf-faster-rcnn/data/crowdai'
        self.label_path = '/Users/anekisei/Documents/tf-faster-rcnn/data/crowdai/Annotations'
        self.image_path = join(self._data_path, 'JPEGImages')
        self.label_matrix = np.genfromtxt(os.path.join(self.label_path, 'labels.csv'),delimiter=',',dtype='str')
        self.file_names = [ f for f in listdir(self.image_path) if isfile(join(self.image_path,f)) ]
        self._classes = {'__background__': '0', 'Car': '1', 'Truck' : '2', 'Pedestrian' : '3'}
    ...
    '''
    def _load_kitti_annotation(self, index):#modify to list[matrix] in the future
        ...
        for i in range(count):
            img_name = label_matrix[index_list[i],4]
