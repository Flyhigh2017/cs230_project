...
import model.vertical_crop as vc
import cv2
import numpy as np
try:
...
def vis_detections(im, class_name, dets, thresh=0.5):
    ...
    for i in range(dets.shape[0]):
        ...
        #for horizontal: store.append((center_x,center_y,x1,x2,y1,y2,scores))
        #cv2.rectangle(im, (x1-10, y1-10), (x2+10, y2+10), (0, 255, 0), 2)
        cv2.circle(im, (center_x, center_y), 3, (0, 255, 0), -1)
        cv2.putText(im, class_name , (center_x - 10, center_y - 5), cv2.FONT_HERSHEY_SIMPLEX, 0.5, (0, 0, 255), 1, cv2.CV_AA)
    # sort verticals
    vertical_points = sorted(vertical_points, key=lambda vertical_points: vertical_points[1], reverse=False)#[:7] up to down
    '''
    for i in range(len(vertical_points)):
        classification = bones[i]
        center_x,center_y = vertical_points[i]
        cv2.putText(im, classification , (center_x - 10, center_y - 5), cv2.FONT_HERSHEY_SIMPLEX, 0.5, (255, 0, 0), 1, cv2.CV_AA)
    '''
...
def Draw_bone(img, vertical_points):
    ...
    for i in range(len(vertical_points)):
        ...
        cv2.putText(img, classification , (center_x - 10, center_y - 5), cv2.FONT_HERSHEY_SIMPLEX, 0.5, (0, 255, 0), 1, cv2.CV_AA)
    return img
